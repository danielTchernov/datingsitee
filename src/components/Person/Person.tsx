...
function Person(props: IPerson) {
  ...
  async function likeAPerson(props: IPerson){
    ...
    try{
      ...
      usersList.filter(user => user.id != props.id)
      let userToShow = usersList[Math.floor((Math.random()* usersList.length))];
      dispatch({type: ActionType.SaveUsersList, payload: {usersList}});
